...
class AppSettingsService extends ChangeNotifier {
  ...
  bool get anonymousReportMode => _boolValue('anonymousReportMode');
  ...
  Future<void> reset() async {
    ...
    await _firestore.collection('users').doc(user.uid).set({
      'appSettings': Map<String, dynamic>.from(defaults),
    }, SetOptions(merge: true));
  }
  ...
  void _listenForUserSettings(User? user) {
    ...
    _settingsSubscription = _firestore
        ...
        .listen((snapshot) {
          ...
          final savedSettings = saved is Map
